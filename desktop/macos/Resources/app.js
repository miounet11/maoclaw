...
const V2_COPY = {
  en: {
    ...
    setupEyebrow: "Stitch-style rebuild",
    setupTitle: "One focused client, rebuilt around the conversation instead of scattered control panels.",
    setupBody: "This version keeps chat centered, moves operations into clear side surfaces, and preserves the bridge that already talks to the local runtime.",
    setupCardTitle: "Finish desktop setup",
    setupCardBody: "Configure one provider, one default model, and one starter agent. The local chat surface comes up immediately after save.",
    ...
    setupNoteA: "The old native shell is no longer the primary path. The host now opens the bundled V2 control center first.",
    setupNoteB: "The hosted web workspace is still available as a secondary surface when you want to leave the local client.",
    setupNoteC: "This design intentionally cuts back tab sprawl and keeps high-value actions within one screen.",
    ...
    statusSetup: "Finish setup to start chatting.",
    ...
    defaultsHint: "Changing defaults updates the next new session and restarts the desktop backend.",
    activeSession: "Active session",
    starterAgentHint: "New sessions start on this lane unless you override the thread later.",
    credentialStateSaved: "Saved credential detected.",
    credentialStateMissing: "No saved credential detected.",
    statsPending: "pending writes",
    statsTools: "tool calls",
    statsMode: "durability",
    setupProviderHint: "Preset routes can still be customized if you need a different endpoint or provider id.",
  ...
  zh: {
    ...
    setupEyebrow: "Stitch 风格重构",
    setupTitle: "把客户端重做成围绕对话展开，而不是到处都是控制页。",
    setupBody: "这个版本把聊天放在中心，把运维和设置放到两侧，并继续复用已经接通本地 runtime 的 bridge。",
    setupCardTitle: "完成桌面端配置",
    setupCardBody: "设置一个 provider、一个默认模型和一个默认 agent。保存后会直接进入本地聊天界面。",
    ...
    saveAndLaunch: "保存并启动",
    saveDefaults: "保存默认值",
    setupNoteA: "旧的 native shell 不再是主路径。宿主应用现在优先打开内置的 V2 控制中心。",
    setupNoteB: "托管 web workspace 仍然保留，可作为次级界面使用。",
    setupNoteC: "这次重写明确减少标签页扩散，把高频动作压回一个主界面。",
    sidebarTitle: "会话",
    newSession: "新会话",
    ...
    defaultsHint: "修改默认值会影响下一个新会话，并重启桌面后端。",
    activeSession: "当前会话",
    starterAgentHint: "新会话默认从这个 lane 启动，之后也可以按会话覆盖。",
    credentialStateSaved: "已检测到保存的凭证。",
    credentialStateMissing: "还没有保存的凭证。",
    statsPending: "待落盘",
    statsTools: "工具调用",
    statsMode: "持久化模式",
    setupProviderHint: "预设路由仍然可以继续改 endpoint 或 provider id。",
    noLogs: "还没有诊断日志。",
